...
public class GraphicsDisplay extends JPanel {
    ...
    private Point2D.Double selectionStart = new Point2D.Double();
	...
	private boolean isDraggingAltGraphicsData = false;
	...
	protected void paintSelection(Graphics2D canvas) {
		if (isSelecting) {
            canvas.setColor(new Color(0, 0, 255, 150));
	...
	protected void drawY(Graphics2D canvas) {
	    ...
	    if(viewport[0][1] > 1) {
	    Point2D.Double markPosition = xyToPoint(0, 1);
	    canvas.draw(new Line2D.Double(
	            markPosition.getX() - 5, markPosition.getY(),
	            markPosition.getX() + 5, markPosition.getY()
	    ));

	    Font smallFont = axisFont.deriveFont(axisFont.getSize() * 0.75f);
	...
	protected void drawX(Graphics2D canvas) {
	    ...
	    if(viewport[1][0] > 1) {
	    Point2D.Double markPosition = xyToPoint(1, 0);
	    canvas.draw(new Line2D.Double(
	            markPosition.getX(), markPosition.getY() - 5,
	            markPosition.getX(), markPosition.getY() + 5
	    ));
